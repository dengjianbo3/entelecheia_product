...
> **Status**: v0.1 contract for the meeting-creation wizard.
...
> **Consumes**: `01-studio-client-spec.md` §7.1 (`list_projects`), §7.5 (`run_meeting`); `02-platform-shell-spec.md` (composables + routing + permissions); `03-auth-service-spec.md` (`platform:run_meeting`); `09-feature-uploads-spec.md` (`useUpload`, `useUploadHandlers`, `MaterialDescriptor`).
> **Forwards to**: `05-feature-agora-spec.md` (router.push to agora on success).
...
- Supporting components: `<WizardHeader>` (step indicator + cancel), `<WizardNavigation>` (back/next/submit buttons), `<ProjectCard>` (project tile in step 1).
- 3 composables: `useWizardDraft` (reactive draft + sessionStorage), `useWizardProjects` (filtered project list), `useWizardSubmit` (the full submit flow).
...
- Submit sequence: `studio.run_meeting` → `POST /api/meetings/:id/metadata` (best-effort) → clear draft → `router.push('/platform/agora/:id')`.
...
- Sealed error taxonomy at the wizard layer (1 leaf owned + relayed leaves).
- Test matrix per step + per composable + the full submit flow.
...
- Materials buffering / upload UI — `09-feature-uploads-spec.md`. Wizard ONLY consumes `useUpload` for the materials step.
...
- Per-meeting cost ceiling, max-rounds, locale overrides — these are `MeetingRunConfig` knobs (per `01-studio-client-spec.md` §5? actually MeetingRunConfig was removed in the 01 rewrite — studio §3.6 has only `version, topic, materials, user_id`). Wizard accepts no advanced config in v0.1.
...
├── src/
│   ├── index.ts                              # exports WizardView + wizardRoutes + permissions
│   ├── WizardView.vue                        # /platform/wizard top-level
│   ├── components/
│   │   ├── WizardHeader.vue                  # step indicator (1 of 4) + project name + cancel
│   │   ├── WizardNavigation.vue              # back / next / submit buttons
│   │   ├── ProjectCard.vue                   # one project tile in step 1
│   │   └── steps/
│   │       ├── ProjectStep.vue               # step 1: pick a project
│   │       ├── TopicStep.vue                 # step 2: enter topic
│   │       ├── MaterialsStep.vue             # step 3: attach materials (uses 09)
│   │       └── ReviewStep.vue                # step 4: review + submit
│   ├── composables/
│   │   ├── useWizardDraft.ts                 # reactive draft + sessionStorage sync
│   │   ├── useWizardProjects.ts              # filtered project list (vertical-scoped)
│   │   └── useWizardSubmit.ts                # full submit flow + error handling
│   ├── routes.ts
│   ├── permissions.ts
│   └── i18n/
│       ├── zh.json
...
export const wizardRoutes: RouteRecordRaw[] = [
  {
    ...
    name: "wizard",
    component: () => import("./WizardView.vue"),
    meta: {
      required_permissions: ["platform:run_meeting", "platform:list_projects"],
      title_key: "feature.wizard.title",
    },
  },
  {
...
Same component for both routes; `:project_id` variant pre-selects + advances to step 2 on mount.
...
export default defineComponent({
  props: {
    project_id: { type: String, default: null },     // from /:project_id route variant
  },
  setup(props) {
    ...
    const { draft, currentStep, setProject, setTopic, addMaterial, removeMaterial,
      ...
      activeVerticalId: computed(() => activeVertical.value?.vertical_id ?? null),
      initialProjectId: props.project_id,
    });
    ...
    async function onSubmit() {
      const handle = await submit(draft.value);     // throws on fatal failures
      // success: cleared in submit(); navigation also in submit()
    }

    return { draft, currentStep, setProject, setTopic, addMaterial, removeMaterial,
             setStep, isStepValid, projects, isLoadingProjects, projectsError,
             isSubmitting, submitError, onCancel, onSubmit };
  },
});
```
...
Route guard ensures `platform:run_meeting` and (for the `/wizard` no-preselect variant) `platform:list_projects`. Missing permission → redirect to dashboard with toast (per `02` §4.1).
...
interface ProjectStepEmits {
  (e: "select", project_id: string): void;
  (e: "retry"): void;
}
```
...
**Auto-advance.** When mounted with `props.project_id` non-null AND that project exists in the fetched list AND is published: auto-advance to step 2 (skip the manual click).
...
- Multi-line textarea for topic. Placeholder example.
...
- Validation runs on every keystroke (debounced 200 ms for the live "valid?" indicator on the Next button); next-button click does final synchronous check.

**Pre-fill.** If `draft.topic` was set previously (e.g., user went back to step 2 from step 3), re-populate. Otherwise empty.

### 3.3 Step 3 — `<MaterialsStep>`

```typescript
interface MaterialsStepProps {
  attached_materials: AttachedMaterial[];      // see below
}
interface MaterialsStepEmits {
  (e: "add-material", upload: Upload, handler_kind: string): void;
  (e: "remove-material", upload_id: string): void;
}
...
interface AttachedMaterial {
  ...
  handler_kind:        string;                  // for re-display
...
**Behavior.**
- When `<UploadDropZone>` emits `files-dropped`: for each file, call `useUpload` (from 09); on success, push to `attached_materials` via emit `add-material`.
- Remove button: emit `remove-material`; the upload remains in `/api/uploads` (user's library) but is dropped from this draft.

**Validation.**
- Always valid (materials are optional). Step 3 → step 4 button always enabled.

**Visual flag** for in-flight uploads: rows show progress bar; "Next" button disabled while ANY upload is in-flight (prevents user clicking Next before files finish posting; otherwise their `upload_id` would be missing from the draft at submit time).
...
interface ReviewStepEmits {
  (e: "submit"): void;
  (e: "back-to-step", step: 1 | 2 | 3): void;
}
```

**Renders.**
- Heading: "Review and start meeting".
- 3 collapsible review sections:
  1. **Project**: name + description + version + paradigm; "Edit" button → emits `back-to-step(1)`.
...
export function useWizardDraft(opts: {
  activeVerticalId: Ref<string | null>;
  initialProjectId: string | null;
}): {
  draft:           Ref<WizardDraft>;
  currentStep:     ComputedRef<1 | 2 | 3 | 4>;
  setProject:      (project_id: string) => void;
  setTopic:        (v: string) => void;
  addMaterial:     (m: AttachedMaterial) => void;
  removeMaterial:  (upload_id: string) => void;
  setStep:         (step: 1 | 2 | 3 | 4) => void;
  isStepValid:     (step: 1 | 2 | 3 | 4) => ComputedRef<boolean>;
  reset:           () => void;
};
```
...
1. **Initialization.**
   ...
   - If found AND `draft.vertical_id === activeVerticalId.value`: restore.
   - Else: create fresh draft with `project_id = initialProjectId`, `current_step = initialProjectId ? 2 : 1`.
2. **Persistence.** Every mutation (setProject / setTopic / addMaterial / removeMaterial / setStep) writes the full draft to sessionStorage (debounced 100 ms to avoid storms).
3. **Vertical change.** When `activeVerticalId` changes mid-wizard: `reset()` is called automatically; user is bumped to step 1; a toast notifies "Vertical switched; wizard reset" (per `02` §2.4 says vertical switch keeps meeting subscriptions but acknowledges other features may reset).
4. **Stale draft cleanup.** On init, if a restored draft's `created_at` is > 7 days old, discard + start fresh. (Bound on draft lifetime in case sessionStorage outlives intent.)
...
```typescript
export function useWizardProjects(activeVertical: Ref<VerticalManifest | null>): {
  projects:        ComputedRef<ProjectSummary[]>;
  isLoadingProjects: ComputedRef<boolean>;
  projectsError:   ComputedRef<string | null>;
  refresh:         () => Promise<void>;
};
```

**Behavior.** On mount + active-vertical change:
1. Compute `project_id_in = activeVertical.value?.default_project_filter.project_id_in ?? []`.
2. `studio.list_projects({ status: "published", limit: 200 })`.
3. Filter client-side to `spec_id in project_id_in`.
4. Sort by `display_name` ascending.
5. Cache per-vertical for the session.
...
export function useWizardSubmit(): {
  ...
  submitError:  ComputedRef<{ message: string; kind: "recoverable" | "fatal" } | null>;
};
```

**Behavior.** See §5 for the full submit flow.
...
async function submit(draft: WizardDraft): Promise<MeetingHandle> {
  ...
  // Fetch bytes for each upload, parse if structured.
  ...
  for (const m of draft.materials) {
    ...
    const content = m.studio_material_kind === "brief"
      ? await blob.text()                                 // string for brief
      : safeParseStructured(await blob.text(), m.mime_type);   // dict for data (JSON parse if applicable; raw string fallback)
  ...
  try {
    handle = await studio.run_meeting({
      ...
      user_id:    useUser().user.value.user_id,
    });
  } catch (err) {
    isSubmitting.value = false;
    submitError.value = mapStudioErrorToSubmitError(err);   // see §5.2
    throw err;
  }
  ...
  try {
    await postMeetingMetadata(handle.meeting_id, {
      ...
      user_id:     handle.meeting_id ? useUser().user.value.user_id : "",
      topic:       draft.topic,
    ...
    // Agora's MaterialsPanel will show "Materials unavailable" (per 05 §4.4 error state).
    // Toast non-blocking warning to the user.
    useToast().show({
      severity: "warning",
      title:    "feature.wizard.warning.metadata_failed",
    });
    // Do NOT abort navigation.
  }

  // ── Phase 4: clear draft + navigate ───────────────────────────────
  reset();
  isSubmitting.value = false;
  router.push({ name: "agora", params: { meeting_id: handle.meeting_id } });
  return handle;
}
```

### 5.2 Error mapping (studio errors → submit errors)

```typescript
function mapStudioErrorToSubmitError(err: unknown): { message: string; kind: "recoverable" | "fatal"; back_to_step?: 1 | 2 | 3 } {
  ...
  if (err instanceof InvalidArgument) {
    // Most likely topic too long / empty (covered by step 2 validation, defensive)
  ...
  if (err instanceof AuthRequired) {
    // Should not happen mid-wizard (route guard would have caught), defensive
    return { message: t("error.auth.auth_required"), kind: "fatal" };
  }
  // Anything else: generic + treat as fatal
  ...
  body:
    {
      ...
      materials:        list[{
                          ...
                          size_bytes:   int,
                          preview_url:  str | null,         # /api/uploads/:id/preview if previewable
                        }]
    }
  auth: required + permission "platform:run_meeting"
  201:  201 Created (no body)
  400:  InvalidArgument (bad shape)
  401/403: relayed
  409:  MeetingMetadataAlreadyExists (idempotency: same meeting_id POSTed twice)
  500:  InternalError
```

Idempotency: if a row with the same `meeting_id` exists AND the body is byte-identical → 201 (idempotent re-POST). If the body differs → 409 with extras showing what changed. v0.1 wizard does NOT retry the metadata POST on failure (per §5.3 best-effort), so the 409 case is operational debug only.
...
| step 1 mount with preselect | `/wizard/proj-a`, proj-a in list | auto-advances to step 2 | `t_w_step1_preselect_skip` |
...
| step 2 ← 1 back retains topic | type then back to 1 then forward | topic preserved | `t_w_step2_topic_persist` |
| step 3 attach material | drop file | useUpload called; on success row added | `t_w_step3_attach` |
| step 3 remove material | click remove on a row | material removed from draft | `t_w_step3_remove` |
| step 3 next disabled while uploading | upload in flight | Next disabled with helper text | `t_w_step3_uploading_disabled` |
...
| submit happy | full draft + click Start | submitting state → router.push to /agora/:id | `t_w_submit_happy` |
...
| cancel | click Cancel + confirm | draft cleared; router.push to dashboard | `t_w_cancel_confirm` |
| cancel without confirm | click Cancel + dismiss | stay in wizard | `t_w_cancel_dismiss` |
...
| useWizardDraft | mutation persists | sessionStorage updated within 100ms | `t_wd_persist_debounce` |
| useWizardDraft | vertical switch resets | reset called; step=1 | `t_wd_vertical_switch_reset` |
...
| useWizardSubmit | full happy flow | all 4 phases run; navigation called | `t_ws_full_flow` |
...
The meeting STARTED on studio after phase 2; aborting navigation because of phase 3 leaves the user stranded with a meeting they can't reach. Better to navigate + show a warning toast + let MaterialsPanel show degraded UX. Trade-off documented (§5.3).
*Considered and rejected.* **Transactional submit** — would require studio rollback (impossible, meetings are append-only); or aborting nav (worse UX). **Retry phase 3 on failure** — wizard should not block the user; agora can later refresh / engineering can re-populate via script.
...
*Considered and rejected.* **Generic "Back to start"** — bad UX. **No back at all** — user trapped with broken draft.

**Why wizard fetches project list itself (vs. relying on knowledge).**
Wizard opens cold — knowledge may not have run. Fetching projects directly here is the natural path. The two features can share a Pinia store cache in v0.2 if list_projects calls become expensive.
*Considered and rejected.* **Lift project list to a shared store** — premature; v0.2 if needed.

**Why MaterialsStep embeds 09's `<UploadDropZone>` (vs. its own simpler picker).**
Consistency with `/platform/uploads` UX; reuses HandlerPicker so vertical handlers are surfaced; avoids duplicate dropzone code. Cost: tighter coupling between wizard and uploads packages — but they're both platform-features and 02's composable model accepts cross-feature reuse.
...
The draft holds `upload_id`s; if user advances before an upload finishes posting, the upload_id is missing → submit later passes incomplete materials → studio agents have less context. Disabling Next during upload is the simplest invariant.
...
**Why the cancel flow shows a confirm dialog.**
Drafts can have substantial state (long topic + multiple files); accidental cancel loses work. Confirm dialog is standard pattern. ESC dismisses (treats as no-op, not as cancel).
*Considered and rejected.* **Cancel with no confirm** — too easy to lose work.
...
| Spec | Adjustment |
|---|---|
| `02-platform-shell-spec.md` | No change. Composables consumed unchanged. |
...
- [ ] Module layout (§1) covers frontend feature; backend metadata endpoint forward-declared
- [ ] 3 permissions reaffirmed (§1.1) — all already declared upstream
- [ ] 2 routes (§1.2) — bare + preselect
- [ ] All 7 visual states (§2.2) covered including 2 submit-error variants (recoverable / fatal)
- [ ] All 4 step components (§3) with full TS prop / emit signatures + render rules + validation rules
- [ ] All 3 composables (§4) with signatures + behavior + persistence rules
...
- [ ] i18n keys (§8) for every user-visible string; en values
- [ ] Test matrix (§9): components (~24), composables (~12); ≥ 35 rows
